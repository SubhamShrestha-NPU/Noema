...
import { LLMPipeline as LLMPipelineWrapper } from "../addon.js";
...
export class LLMPipeline {
     ...
     */
    async startChat(systemMessage = "") {
        console.warn("DEPRECATION WARNING: startChat() / finishChat() API is deprecated and will be removed in the next major release.", "Please, use generate() with ChatHistory argument.");
        if (!this.pipeline)
            throw new Error("LLMPipeline is not initialized");
     ...
     */
    stream(inputs, generationConfig = {}) {
        if (!this.pipeline)
            throw new Error("LLMPipeline is not initialized");
        if (Array.isArray(inputs))
            throw new Error("Streaming is not supported for array of inputs. Please use LLMPipeline.generate() method.");
        ...
        const callback = (error, result) => {
            if (error) {
                ...
                const decodedResult = new DecodedResults(result.texts, result.scores, result.perfMetrics, result.parsed);
     ...
     */
    async generate(inputs, generationConfig = {}, streamer) {
        if (!this.pipeline)
            throw new Error("LLMPipeline is not initialized");
        if (typeof generationConfig !== "object")
            throw new Error("Options must be an object");
